<template>
  <div>
    ...
    <el-table id="rebateSetTable" v-loading="loading" :data="StatisticsVo" empty-text="暂无数据" class="el-table-1" height="auto" :default-sort = "{ order: 'descending'}">
      ...
      <el-table-column sortable prop="cName" label="客户名称" width="200" align="center"/>
      <el-table-column sortable prop="cLevel" label="客户级别"  align="center">
        <template slot-scope="scope">
          <span v-if='scope.row.cLevel ==1'>初级</span>
          <span v-if='scope.row.cLevel ==2'>中级</span>
          <span v-if='scope.row.cLevel ==3'>高级</span>
        </template>
      </el-table-column>
      <el-table-column sortable prop="cPhone" label="手机号" width="100" align="center"/>
      <el-table-column sortable prop="itemName" label="商品名称"  align="center"/>
      <el-table-column sortable prop="models" label="规格"  align="center"/>
      <el-table-column sortable prop="status" label="操作类型"  align="center">
        <template slot-scope="scope">
          <span v-if='scope.row.status ==0'>出库</span>
          <span v-if='scope.row.status ==1'>退货</span>
          <span v-if='scope.row.status ==2'>入库</span>
        </template>
      </el-table-column>
      <el-table-column sortable prop="createTime" label="操作时间"  align="center"/>
      <el-table-column sortable prop="transportCompany" label="物流"  align="center"/>
      <el-table-column sortable prop="transportNo" label="物流单号"  align="center"/>
    </el-table>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      exportExcel () {
        ...
        try {
          FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '按客户统计'+this.StatisticsQo.startTime+'至'+this.StatisticsQo.endTime+'.xlsx');
        } catch (e)
        {
      ...
      onSubmit() {
        ...
        this.loading = true
        ajaxUtil.get('/statistics/getStatisticsByClient', {
          params: {
